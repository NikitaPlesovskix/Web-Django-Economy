...
from .models import  Case, Section, Parameter, Period, Data, Formula, Variable, Status, Job
...
@admin.register(Formula) #Отображение списка формул
class Formula_Admin(admin.ModelAdmin):
    list_display = ('Formula_ID','parameter','Formula_Name')
    list_display_links = ('Formula_Name','parameter',)
    search_fields = ('Formula_ID','Formula_Name')
    
        
@admin.register(Variable) #Отображение списка Переменых
class Varible_Admin(admin.ModelAdmin): 
    list_display = ('Variable_ID','parameter','Variable_Name')
    list_display_links = ('Variable_Name',)
    search_fields = ('Variable_ID','parameter','Variable_Name')


@admin.register(Status) #Отображение списка Статусов
class Status_Admin(admin.ModelAdmin): 
    list_display = ('Status_ID','Status_Name','Status_Comment','group')
    list_display_links = ('Status_Name','group',)
    search_fields = ('Status_ID','Status_Name','Status_Comment','group')


@admin.register(Job) #Отображение списка кейсов в работе
class Job_Admin(admin.ModelAdmin): 
    list_display = ('Job_ID','case','user','status')
    list_display_links = ('case','user','status',)
    search_fields = ('Job_ID','case','user','status')